...
class CategoryExportController
{
    public function __invoke(
        ...
        $rows = $categoryService->exportRows($filters);
        $columns = $this->columns();
        $filename = 'categories-export-'.now()->format('Y-m-d-His');

        return match ($format) {
            'csv' => $exportService->csv($filename, $columns, $rows),
            'excel' => $exportService->excel($filename, $columns, $rows),
            'pdf' => $exportService->pdf($filename, 'Categories Export', $columns, $rows),
        };
    }
    ...
    private function columns(): array
    {
        return [
            ...
            ['name' => 'products_count', 'label' => 'Products'],
