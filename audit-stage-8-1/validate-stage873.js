...
const screenshotsDir = path.join(__dirname, 'screenshots-stage873');
...
(async () => {
    ...
    await capture(page, 'stage873_customers_list_desktop', true);

    // Capture first customer URL for profile view
    const firstCustomerHref = await page.locator('td a[href*="/admin/customers/"]').first().getAttribute('href').catch(() => null);

    // Filtered customers view (customers without orders)
    await page.goto(`${BASE_URL}/admin/customers?tableFilters[no_orders][isActive]=1`, { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(3000);
    await capture(page, 'stage873_customers_filtered_desktop', true);

    // View first customer directly to avoid session filter persistence side effects
    if (firstCustomerHref) {
        const profileUrl = firstCustomerHref.startsWith('http') ? firstCustomerHref : `${BASE_URL}${firstCustomerHref}`;
        await page.goto(profileUrl, { waitUntil: 'domcontentloaded' });
        await page.waitForURL(url => url.pathname.match(/\/admin\/customers\/\d+$/) !== null, { timeout: 15000, waitUntil: 'domcontentloaded' });
        await page.waitForTimeout(3000);
        await capture(page, 'stage873_customer_profile_desktop', true);
    }
    ...
    await capture(page, 'stage873_customers_list_tablet', true);
    ...
    await capture(page, 'stage873_customers_list_mobile', true);

    await browser.close();

    fs.writeFileSync(path.join(__dirname, 'stage873-validation.json'), JSON.stringify({ consoleErrors, pageErrors }, null, 2));
