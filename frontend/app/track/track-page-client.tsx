...
function TrackingResult({ order }: { order: Order }) {
  ...
  return (
    <div className="space-y-6">
      {/* Status Header */}
      <div className="bg-white rounded-[20px] border border-default shadow-sm p-6 lg:p-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <p className="text-sm text-muted mb-1">Order {order.invoice_number}</p>
            <h2 className="text-2xl font-extrabold text-primary-900 capitalize">{order.status}</h2>
            <p className="text-muted mt-1">{latestEvent?.description}</p>
          </div>
          <div className="text-left md:text-right">
            <p className="text-sm text-muted">Estimated Delivery</p>
            <p className="text-xl font-bold text-primary-500">{formatDate(order.estimated_delivery)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        {/* Timeline */}
        <div className="lg:col-span-2 bg-white rounded-[20px] border border-default shadow-sm p-6">
          <h3 className="text-lg font-bold text-primary-900 mb-6">Shipment Timeline</h3>
        ...
        {/* Details */}
        <div className="space-y-6">
          <div className="bg-white rounded-[20px] border border-default shadow-sm p-6">
            <div className="flex items-center gap-2 mb-4">
              <Truck className="w-5 h-5 text-green-600" />
              <h3 className="text-lg font-bold text-primary-900">Tracking Details</h3>
            </div>
            <div className="text-sm text-muted space-y-2">
              ...
              <div className="flex justify-between">
                <span>Delivered</span>
                <span className="font-medium text-primary-900">{formatDate(order.delivered_at)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[20px] border border-default shadow-sm p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-bold text-primary-900">Shipping Address</h3>
            </div>
          ...
          <Link
            ...
            className="flex items-center justify-center gap-2 w-full px-4 py-3 bg-green-600 text-white font-semibold rounded-xl hover:bg-green-700 transition-colors-base"
          >
    ...
    <>
      ...
      <section className="py-12 lg:py-20 bg-surface-page">
        <Container className="max-w-4xl">
          ...
          <div className="bg-white rounded-[20px] border border-default shadow-sm p-6 lg:p-8 mb-8">
            <form
              onSubmit={(e) => {
                e.preventDefault();
            ...
            >
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 rounded-xl border border-default focus:outline-none focus:ring-2 focus:ring-green-500"
                />
              </div>
              <button
                type="submit"
                disabled={ordersLoading}
                className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-green-600 text-white font-semibold rounded-xl hover:bg-green-700 disabled:opacity-60 disabled:cursor-not-allowed transition-colors-base"
              >
          ...
          {ordersLoading ? (
            ...
            <div className="bg-white rounded-[20px] border border-default shadow-sm p-12 text-center">
              <AlertCircle className="w-14 h-14 mx-auto mb-4 text-placeholder" />
              <h3 className="text-lg font-bold text-primary-900 mb-2">Order not found</h3>
