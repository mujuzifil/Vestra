            ...
            {isLoading ? (
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="h-32 rounded-[20px] bg-neutral-200 animate-pulse" />
                ))}
              </div>
            ) : error ? (
        ...
        <section className="py-16 lg:py-24 bg-surface-page" aria-labelledby="contact-form-heading">
          <Container>
            <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
              <AnimatedSection
                direction="left"
                className="p-6 lg:p-10 rounded-[24px] bg-white border border-default shadow-lg"
              >
                <h2
                  id="contact-form-heading"
                  className="text-xl lg:text-2xl font-bold text-primary-900 mb-2"
                >
                  Send us a Message
                </h2>
              ...
              <AnimatedSection
                direction="right"
                className="rounded-[24px] overflow-hidden border border-default bg-white shadow-lg h-full min-h-[360px] flex flex-col"
              >
                <div className="p-6 border-b border-default">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-muted">{contactInfo?.location || "Kampala, Uganda"}</p>
                    </div>
                  </div>
                </div>
                <div className="flex-1 bg-neutral-200 flex items-center justify-center p-8">
                  <div className="text-center">
                    <MapPin className="w-12 h-12 text-placeholder mx-auto mb-3" aria-hidden="true" />
                    <p className="text-muted font-medium">Google Maps placeholder</p>
                    <p className="text-sm text-placeholder">{contactInfo?.location || "Kampala, Uganda"}</p>
                  </div>
                </div>
              </AnimatedSection>
            </div>
