  ...
  return (
    <div className="bg-surface-page rounded-xl p-4 sm:hidden">
      ...
      <div className="flex items-center justify-between mb-3">
        <p className="font-bold text-primary-500">UGX {order.total_amount}</p>
        <PaymentStatusBadge status={order.payment_status} />
      </div>
      <Link
        href={`/account/orders/${order.id}`}
        className="w-full inline-flex items-center justify-center gap-1.5 text-sm font-semibold text-secondary-600 hover:text-secondary-600 bg-surface-card border border-default rounded-lg py-2"
      >
        ...
        <Container>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
            ...
            <div className="bg-surface-card rounded-[20px] border border-default shadow-sm p-4">
              ...
              <p className="text-2xl font-extrabold text-warning-600">{stats.pendingPayment}</p>
            </div>
            <div className="bg-surface-card rounded-[20px] border border-default shadow-sm p-4">
              <p className="text-sm text-muted">Processing</p>
              <p className="text-2xl font-extrabold text-info-600">{stats.processing}</p>
            </div>
            <div className="bg-surface-card rounded-[20px] border border-default shadow-sm p-4">
              <p className="text-sm text-muted">Completed</p>
              <p className="text-2xl font-extrabold text-secondary-600">{stats.completed}</p>
            </div>
            <div className="bg-surface-card rounded-[20px] border border-default shadow-sm p-4 col-span-2 lg:col-span-1">
              <p className="text-sm text-muted">Cancelled / Refunded</p>
              <p className="text-2xl font-extrabold text-danger-600">{stats.cancelled}</p>
            </div>
          </div>
            ...
            <div className="flex flex-col lg:flex-row gap-3 mb-6">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-default text-sm focus:outline-none focus:ring-2 focus:ring-secondary-500"
                />
              </div>
              <div className="flex gap-3">
                <div className="relative">
                  <SlidersHorizontal className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-placeholder" />
                  <select
                    value={statusFilter}
                    onChange={(e) => setStatusFilter(e.target.value)}
                    className="pl-10 pr-4 py-2.5 rounded-xl border border-default text-sm focus:outline-none focus:ring-2 focus:ring-secondary-500 bg-surface-card"
                  >
                    {statusOptions.map((o) => (
                ...
                <div className="relative">
                  <CreditCard className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-placeholder" />
                  <select
                    value={paymentFilter}
                ...
                <div className="relative">
                  <ArrowUpDown className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-placeholder" />
                  <select
                    value={sort}
