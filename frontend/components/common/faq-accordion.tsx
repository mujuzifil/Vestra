      ...
      {items.map((item, index) => {
        ...
        return (
          ...
          >
            ...
            >
              <span className="font-semibold text-text-heading text-base lg:text-lg">{item.question}</span>
              <span
                className={cn(
                  "w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-colors-base",
                  isOpen ? "bg-secondary-500 text-white" : "bg-neutral-100 text-text-muted"
                )}
              >
            ...
            <AnimatePresence initial={false}>
              {isOpen && (
                ...
                >
                  <p className="px-5 lg:px-6 pb-5 lg:pb-6 text-text-body leading-relaxed">{item.answer}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
